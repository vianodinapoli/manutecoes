            ...
            <div class="card-body p-4">
                <form action="{{ route('compras.update', $compra->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row g-3 mb-4 p-3 bg-light rounded border">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="metadata[urgencia]" id="urgencia" class="form-select">
                                <option value="Normal"
                                    {{ ($compra->metadata['urgencia'] ?? '') == 'Normal' ? 'selected' : '' }}>Normal
                                </option>
                                <option value="Alta"
                                    {{ ($compra->metadata['urgencia'] ?? '') == 'Alta' ? 'selected' : '' }}>Alta - Carro
                                    parado na oficina</option>
                                <option value="Crítica"
                                    {{ ($compra->metadata['urgencia'] ?? '') == 'Crítica' ? 'selected' : '' }}>Crítica -
                                    Aguardando peça para entrega</option>
                            </select>
                        </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-6">
                            <label for="quotation" class="form-label">Substituir Cotação (Deixe vazio para
                                manter)</label>
                            <input type="file" name="quotation_file" id="quotation_file" class="form-control">
                            @if ($compra->quotation_file)
                                <div class="mt-2">
                                    <small class="text-success">✅ Ficheiro atual: <a
                                            href="{{ asset('storage/' . $compra->quotation_file) }}"
                                            target="_blank">Ver Anexo</a></small>
                                </div>
                            @endif
                        </div>
                    </div>
