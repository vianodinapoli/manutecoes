...
<style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* ── Stock badge inline ── */
    .stock-pill { display:inline-flex; align-items:center; padding:2px 8px; border-radius:20px; font-size:.68rem; font-weight:700; }
    .stock-pill.ok  { background:#f0fdf4; color:#166534; border:1px solid #bbf7d0; }
    .stock-pill.low { background:#fefce8; color:#92400e; border:1px solid #fde68a; }
    .stock-pill.out { background:#fef2f2; color:#991b1b; border:1px solid #fecaca; }
    ...
    /* ── Alert ── */
    .alert-success { background:#f0fdf4; border:1px solid #bbf7d0; color:#166534; border-radius:8px; padding:10px 16px; font-size:.8rem; font-weight:600; display:flex; align-items:center; gap:8px; margin-bottom:16px; }
    .alert-error   { background:#fef2f2; border:1px solid #fecaca; color:#991b1b; border-radius:8px; padding:10px 16px; font-size:.8rem; font-weight:600; display:flex; align-items:center; gap:8px; margin-bottom:16px; }
    ...
    @media (max-width: 768px) {
        ...
        .mov-table th:nth-child(5), .mov-table td:nth-child(5) { display:none; } /* obs */
        .page-header { flex-direction:column; align-items:flex-start; }
    }
    @media (max-width: 480px) {
        .kpi-grid { grid-template-columns:1fr 1fr 1fr; gap:6px; }
        .kpi-value { font-size:1.1rem; }
        .kpi-icon { width:30px; height:30px; font-size:.8rem; }
        .filter-grid { grid-template-columns:1fr; }
        .mov-table th:nth-child(3), .mov-table td:nth-child(3) { display:none; } /* responsável */
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-weight:700;font-size:.85rem;">
                            ...
                            <span style="font-size:.7rem;color:#94a3b8;font-weight:400;">{{ $mov->stockItem->unidade ?? '' }}</span>
                        </td>
                        <td>
                            <div style="display:flex;align-items:center;gap:7px;">
                                <div class="avatar">{{ strtoupper(substr($mov->responsavel, 0, 1)) }}</div>
                                <span style="font-size:.78rem;">{{ $mov->responsavel }}</span>
                            </div>
                        </td>
                        <td><span class="obs-text" title="{{ $mov->observacoes }}">{{ $mov->observacoes ?: '—' }}</span></td>
                        <td>
                        ...
                        <td class="text-end">
                            <div style="display:flex;justify-content:flex-end;gap:4px;">
                                {{-- Editar (só responsável e obs) --}}
                                <button class="btn-outline-sm"
                                    title="Editar"
                                    onclick="abrirModalEditar({{ $mov->id }}, '{{ addslashes($mov->responsavel) }}', '{{ addslashes($mov->observacoes) }}')">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                {{-- Anular --}}
                                <button class="btn-danger-sm"
                                    title="Anular movimento"
                                    onclick="confirmarAnulacao({{ $mov->id }}, '{{ $mov->tipo === 'entrada' ? 'entrada' : 'saída' }}', '{{ addslashes($mov->stockItem->nome ?? '') }}', {{ $mov->quantidade }})">
        ...
        <form method="POST" action="{{ route('movimentos.store') }}" id="formNovo">
            ...
            <div class="modal-body">
                ...
                <div style="margin-bottom:14px;">
                    ...
                    <select name="stock_item_id" id="stock_item_id" class="form-control-custom" onchange="carregarStock(this.value)" required>
                        <option value="">— Selecione um produto —</option>
                        @foreach($stockItems as $p)
                       <option value="{{ $p->id }}" data-stock="{{ $p->quantidade }}" data-unidade="">
                ...
                <div class="form-row-2" style="margin-bottom:14px;">
                    <div>
                        ...
                        <input type="number" name="quantidade" id="quantidade" class="form-control-custom"
                               min="0.01" step="0.01" placeholder="0.00" required>
                    </div>
                    <div>
                        <label class="form-label" for="responsavel">Responsável</label>
                        <input type="text" name="responsavel" id="responsavel" class="form-control-custom"
                               placeholder="Nome do serralheiro…" required>
                    </div>
                </div>

                {{-- OBSERVAÇÕES --}}
                <div>
                    <label class="form-label" for="observacoes">Observações <span style="font-weight:400;text-transform:none;letter-spacing:0;">(opcional)</span></label>
                    <textarea name="observacoes" id="observacoes" class="form-control-custom"
                              rows="2" placeholder="Ex: Entregue para reparação da máquina X…"></textarea>
                </div>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn-confirm-cancel" onclick="fecharModal('modalNovo')">Cancelar</button>
                <button type="submit" class="btn-primary-dark" id="btn-submit-novo">
                    <i class="bi bi-check-lg"></i> <span id="btn-submit-label">Registar Entrada</span>
                </button>
            </div>
        </form>
    </div>
...
{{-- ═══════════════════════════════════════════════
     MODAL — EDITAR (só responsável + obs)
═══════════════════════════════════════════════ --}}
        ...
        <form method="POST" id="formEditar">
            ...
            <div class="modal-body">
                <div style="margin-bottom:14px;">
                    ...
                    <input type="text" name="responsavel" id="edit-responsavel" class="form-control-custom" required>
                </div>
                <div>
                    <label class="form-label" for="edit-observacoes">Observações</label>
                    <textarea name="observacoes" id="edit-observacoes" class="form-control-custom" rows="3"></textarea>
                </div>
            </div>
            <div class="modal-footer">
...
<script>
    ...
    function selecionarTipo(tipo) {
        ...
        document.getElementById('btn-entrada').classList.toggle('entrada',  tipo === 'entrada');
        document.getElementById('btn-saida').classList.toggle('selected',   tipo === 'saida');
        document.getElementById('btn-saida').classList.toggle('saida',      tipo === 'saida');

        var label = tipo === 'entrada' ? 'Registar Entrada' : 'Registar Saída';
        document.getElementById('btn-submit-label').textContent = label;

        // Mostrar/esconder stock disponível (mais relevante para saídas)
    ...
    // ── Carregar stock do produto (por select change) ─────
    function carregarStock(produtoId) {
        ...
        // Ler do option (sem AJAX, dados já estão no DOM)
        ...
        if (option) {
            ...
            valorEl.textContent   = stock.toLocaleString('pt-PT', {minimumFractionDigits: 2, maximumFractionDigits: 2});
            unidadeEl.textContent = unidade;
            wrap.classList.add('show');
            wrap.dataset.loaded = '1';

            // Colorir consoante stock
            valorEl.style.color = stock <= 0 ? '#dc2626' : stock < 5 ? '#f59e0b' : '#16a34a';
        }
    }

    // ── Abrir modal editar ───────────────────────────────
    function abrirModalEditar(id, responsavel, observacoes) {
        var baseUrl = "{{ url('/movimentos') }}/" + id;
    ...
    function confirmarAnulacao(id, tipo, produto, quantidade) {
        var baseUrl = "{{ url('/movimentos') }}/" + id;
        document.getElementById('formAnular').action = baseUrl;

        var qtdFormatada = parseFloat(quantidade).toLocaleString('pt-PT', {minimumFractionDigits: 2, maximumFractionDigits: 2});
        var acao = tipo === 'entrada' ? 'retirado do stock' : 'devolvido ao stock';
        document.getElementById('confirm-desc').innerHTML =
            'Vai anular <strong>' + tipo + '</strong> de <strong>' + qtdFormatada + '</strong> × <strong>' + produto + '</strong>.<br>' +
