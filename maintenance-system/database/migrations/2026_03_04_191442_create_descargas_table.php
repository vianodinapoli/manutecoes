...
<style>
    ...
    .filter-title  { font-size:0.72rem; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; color:#adb5bd; margin-bottom:14px; display:flex; align-items:center; gap:8px; }
    ...
    .filter-item select, .filter-item input { border:1px solid #dee2e6; border-radius:8px; padding:7px 12px; font-size:0.82rem; color:#343a40; background:#f8f9fa; outline:none; transition:border-color 0.2s,box-shadow 0.2s; min-width:120px; }
    ...
    /* Calculadora kg */
    .calc-block { display:flex; align-items:flex-end; gap:8px; padding:10px 14px; border-radius:10px; }
    .calc-block.alta  { background:#eef3ff; border:1px solid #c7d9ff; }
    .calc-block.baixa { background:#fff0f0; border:1px solid #ffc9c9; }
    .calc-label { font-size:0.62rem; font-weight:700; letter-spacing:1px; text-transform:uppercase; margin-bottom:4px; }
    .calc-label.alta  { color:#1a56db; }
    .calc-label.baixa { color:#dc3545; }
    .calc-result { border-radius:8px; padding:5px 12px; font-size:0.8rem; font-weight:700; min-width:110px; text-align:center; }
    .calc-result.alta  { background:#e8f0fe; color:#1a56db; border:1px solid #b8d4f8; }
    .calc-result.baixa { background:#fde8e8; color:#dc3545; border:1px solid #f8b8b8; }
    .calc-result small { display:block; font-size:0.6rem; font-weight:500; opacity:0.65; }
    .calc-total { background:#f0fdf4; border:1px solid #a3e6c5; border-radius:8px; padding:5px 14px; font-size:0.8rem; font-weight:700; color:#198754; text-align:center; min-width:110px; }
    .calc-total small { display:block; font-size:0.6rem; font-weight:500; opacity:0.7; }
    ...
    /* Pills de sacos — compactas */
    .sacos-wrap { display:flex; flex-direction:column; gap:3px; }
    .saco-pill  { display:inline-flex; align-items:center; gap:4px; border-radius:5px; padding:2px 7px; font-size:0.72rem; font-weight:700; width:fit-content; line-height:1.3; }
    ...
    .saco-pill .pill-type { font-size:0.6rem; font-weight:700; letter-spacing:0.5px; text-transform:uppercase; opacity:0.7; }
    .saco-pill .pill-n    { font-size:0.82rem; font-weight:800; }

    /* Divergência */
    .divergence-badge { display:inline-flex; align-items:center; gap:4px; background:#fff3cd; color:#856404; border:1px solid #ffc107; border-radius:20px; padding:2px 7px; font-size:0.6rem; font-weight:600; margin-top:2px; }
...
<div class="container-fluid py-4 px-4">
    ...
    {{-- KPI CARDS --}}
    @php
        $pesoTotalCalculado = $discharges->sum(fn($d) =>
            (($d->sacos_alta ?? 0) * ($d->peso_saco_alta ?? 0)) +
            (($d->sacos_baixa ?? 0) * ($d->peso_saco_baixa ?? 0))
        );
    @endphp
    <div class="row g-3 mb-3 no-print">
        ...
        <div class="col-6 col-md-2">
            <div class="kpi-card gold">
                <div class="kpi-label">Peso Calculado</div>
                <div class="kpi-value" style="color:#fd7e14;font-size:1rem;padding-top:4px;">
                    {{ number_format($pesoTotalCalculado / 1000, 1, ',', '.') }}t
                </div>
                <div class="kpi-sub">sacos × peso registo</div>
                <i class="bi bi-calculator kpi-icon" style="color:#fd7e14;"></i>
            </div>
        </div>
    </div>
    ...
    <div class="filter-panel no-print">
        ...
        {{-- Linha 2: Calculadora kg (ao vivo) --}}
        <div class="d-flex flex-wrap gap-3 align-items-center">

            {{-- ALTA --}}
            <div class="calc-block alta">
                <div>
                    <div class="calc-label alta"><i class="bi bi-arrow-up-circle-fill me-1"></i>Kg por Saco Alta</div>
                    <input type="number" id="kgAlta" placeholder="ex: 50" min="1" step="0.1"
                           style="border:1px solid #b8d4f8;border-radius:8px;padding:6px 10px;font-size:0.82rem;background:#fff;outline:none;width:90px;"
                           oninput="calcKg()">
                </div>
                <div class="calc-result alta" id="resultAlta">
                    <small>Total Alta</small>
                    <span id="resultAltaKg">— kg</span>
                    <small id="resultAltaSacos" style="opacity:0.5;">0 sacos</small>
                </div>
            </div>

            {{-- BAIXA --}}
            <div class="calc-block baixa">
                <div>
                    <div class="calc-label baixa"><i class="bi bi-arrow-down-circle-fill me-1"></i>Kg por Saco Baixa</div>
                    <input type="number" id="kgBaixa" placeholder="ex: 25" min="1" step="0.1"
                           style="border:1px solid #f8b8b8;border-radius:8px;padding:6px 10px;font-size:0.82rem;background:#fff;outline:none;width:90px;"
                           oninput="calcKg()">
                </div>
                <div class="calc-result baixa" id="resultBaixa">
                    <small>Total Baixa</small>
                    <span id="resultBaixaKg">— kg</span>
                    <small id="resultBaixaSacos" style="opacity:0.5;">0 sacos</small>
                </div>
            </div>

            {{-- TOTAL combinado (só aparece quando ambos preenchidos) --}}
            <div class="calc-total" id="calcTotalBox" style="display:none;">
                <small>Total Combinado</small>
                <span id="calcTotalKg">—</span>
            </div>
            ...
            <table class="table table-hover align-middle mb-0" id="dischargesTable" style="width:100%">
                ...
                <tbody>
                    ...
                    >
                        ...
                        {{-- SACOS — pill compacta: "ALTA 12" ou "BAIXA 8" --}}
                        <td>
                            <div class="sacos-wrap">
                                @if($sacosAlta > 0)
                                <span class="saco-pill alta">
                                    <span class="pill-type">Alta</span>
                                    <span class="pill-n">{{ $sacosAlta }}</span>
                                </span>
                                @endif
                                @if($sacosBaixa > 0)
                                <span class="saco-pill baixa">
                                    <span class="pill-type">Baixa</span>
                                    <span class="pill-n">{{ $sacosBaixa }}</span>
                                </span>
                                @endif
                                @if($hasDivergence)
                                <span class="divergence-badge">
                                    <i class="bi bi-exclamation-triangle-fill"></i> Conf: {{ $discharge->sacos_confirmados }}
                                </span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="text-center">
                            @if($discharge->tempo_transporte)
                                <span class="badge bg-info bg-opacity-15 text-info fw-semibold" style="font-size:0.68rem;">
    ...
    <div style="font-family:Arial,sans-serif;padding:20px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px;">
            ...
            <div style="background:#fff7e6;border-left:4px solid #fd7e14;border-radius:8px;padding:12px;">
                <div style="font-size:0.65rem;color:#666;text-transform:uppercase;font-weight:bold;">Peso Calculado Total</div>
                <div style="font-size:1.1rem;font-weight:bold;color:#fd7e14;">{{ number_format($pesoTotalCalculado,0,',','.') }} kg</div>
            </div>
        </div>
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:24px;font-size:0.8rem;">
            <thead>
                <tr>
                    ...
                    <th style="padding:9px 12px;text-align:center;background:#198754;color:white;">Total Sacos</th>
                    <th style="padding:9px 12px;text-align:center;background:#fd7e14;color:white;">Peso Calculado</th>
            ...
            <tbody>
                ...
                @php
                    $gAlta  = $grupo->sum('sacos_alta');
                    $gBaixa = $grupo->sum('sacos_baixa');
                    $gTotal = $gAlta + $gBaixa;
                    $gPeso  = $grupo->sum(fn($d) => (($d->sacos_alta??0)*($d->peso_saco_alta??0))+(($d->sacos_baixa??0)*($d->peso_saco_baixa??0)));
                @endphp
                <tr style="border-bottom:1px solid #eee;{{ $loop->even ? 'background:#f9f9f9;' : '' }}">
                    ...
                    <td style="padding:8px 12px;text-align:center;color:#fd7e14;font-weight:bold;">{{ number_format($gPeso,0,',','.') }} kg</td>
            ...
            <tfoot>
                ...
                <tr style="background:#f0fdf4;font-weight:bold;border-top:2px solid #198754;">
                    ...
                    <td style="padding:8px 12px;text-align:center;color:#fd7e14;">{{ number_format($pesoTotalCalculado,0,',','.') }} kg</td>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:0.75rem;">
            <thead>
                <tr style="background:#1a56db;color:white;">
                    ...
                    <th style="padding:7px 10px;text-align:center;background:#e65100;">Peso Calc.</th>
            ...
            <tbody>
                ...
                @php
                    $dA=$d->sacos_alta??0; $dB=$d->sacos_baixa??0;
                    $dP=($dA*($d->peso_saco_alta??0))+($dB*($d->peso_saco_baixa??0));
                @endphp
                <tr style="border-bottom:1px solid #eee;{{ $loop->even ? 'background:#f9f9f9;' : '' }}">
                    ...
                    <td style="padding:6px 10px;text-align:center;color:#fd7e14;font-weight:bold;">{{ number_format($dP,0,',','.') }} kg</td>
...
// ── Calculadora kg — usa sacos das linhas visíveis ──
function calcKg() {
    const kgA = parseFloat($('#kgAlta').val()) || 0;
    const kgB = parseFloat($('#kgBaixa').val()) || 0;

    let visA = 0, visB = 0;
    $('#dischargesTable tbody tr:visible').each(function () {
        visA += parseInt($(this).data('sacos-alta'))  || 0;
        visB += parseInt($(this).data('sacos-baixa')) || 0;
    });

    // Alta
    $('#resultAltaSacos').text(visA.toLocaleString('pt-PT') + ' sacos');
    $('#resultAltaKg').text(kgA > 0 ? (visA * kgA).toLocaleString('pt-PT') + ' kg' : '— kg');

    // Baixa
    $('#resultBaixaSacos').text(visB.toLocaleString('pt-PT') + ' sacos');
    $('#resultBaixaKg').text(kgB > 0 ? (visB * kgB).toLocaleString('pt-PT') + ' kg' : '— kg');

    // Total combinado
    if (kgA > 0 && kgB > 0) {
        $('#calcTotalBox').show();
        $('#calcTotalKg').text(((visA * kgA) + (visB * kgB)).toLocaleString('pt-PT') + ' kg');
    } else {
        $('#calcTotalBox').hide();
    }
}
...
function applyFilters() {
    ...
    renderTags(df, dt, tr, st, tc, div);
    dtTable.draw();
    calcKg(); // actualiza calculadora com linhas filtradas
}

function clearFilters() {
    $('#filterDateFrom,#filterDateTo').val('');
    $('#filterTransportadora,#filterStatus,#filterTipoCarga,#filterDivergencia').val('');
    $('#dischargesTable tbody tr').show();
    $('#activeTags').html('');
    dtTable.draw();
    calcKg();
}
...
function renderTags(df,dt,tr,st,tc,div) {
    ...
    const l={df:`De: ${df}`,dt:`Até: ${dt}`,tr:`Transp: ${tr}`,st:`Estado: ${st}`,tc:`Carga: ${tc}`,div:`Diverg: ${div}`};
    let html='';
